...
- Ingestion job: pull weekly shoprite_sales output → normalize → load into Postgres (schema in §3).
...
- **First task before any of this:** a data-profiling script that runs against one week of scraper output and reports: field completeness, name/size format variance, price parsing failures, category coverage. This determines how much normalization work phases 2–4 need.
...
1. Weekly cron triggers scrape (existing Scrapfly job) → output lands as a batch.
2. Loader writes every row verbatim to `raw_sale_items` (never lose the original).
...
- **Frontend:** Next.js (App Router), TypeScript, Tailwind CSS. Server components for deals/recipe pages (SEO matters — "shoprite sale this week" is organic traffic), client components for planner interactivity.
- **API:** REST or tRPC endpoints exposed by the valueplate services; frontend never queries Postgres directly.
- **DB:** Postgres (managed — Neon/Supabase/RDS per existing valueplate infra).
- **Jobs:** weekly scrape + ingest as a scheduled job (existing scraper cadence).
...
1. **Profile the data.** Script that ingests one week of shoprite_sales output and reports field completeness, name/size parse rate, price format inventory, category coverage. Everything downstream depends on this.
2. Stand up Postgres schema (§3) + migration tooling.
3. Build loader (raw → `raw_sale_items`) and normalizer v1 (target: ≥90% of rows parsed cleanly; report the rest).
4. Next.js app skeleton + deals browse page against real data.
5. Unit-price computation + sort/filter/search.
6. Then proceed through phases 3–4 as vertical slices.

### Open questions to resolve while building
- What exactly does shoprite_sales output per item today (fields, formats, category labels)? → answered by task 1.
- Does the scrape carry regular prices, or sale prices only? Savings math depends on this.
- Store-level vs. chain-level pricing: are ShopRite circulars regional? If so, `stores.location` matters sooner.
